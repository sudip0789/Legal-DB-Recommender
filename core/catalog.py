"""
Loads catalog.json and research_guides.json and builds the system prompt once
at module import time. SYSTEM_PROMPT is a module-level constant — byte-identical
across all requests, which is required for Anthropic prompt caching to produce
cache hits.
"""
...
_GUIDES_PATH = _BASE / "data" / "research_guides.json"
...
with open(_GUIDES_PATH, encoding="utf-8") as _f:
    GUIDES: dict = json.load(_f)
...
# Inject the full catalog and guides as compact JSON (saves tokens; model
# handles it fine).
